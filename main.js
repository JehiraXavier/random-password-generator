...
function applyFill(slider) {
	...
	const bg = `linear-gradient(90deg, ${slider_props.fill} ${percentage}%, ${slider_props.background} ${percentage +
			0.1}%)`;
...
generate_button.addEventListener("click", () => {
	...
	result_element.innerText = generatePassword(length, hasLower, hasUpper, hasNumber, hasSymbol);
...
function generatePassword(length, lower, upper, number, symbol) {
	...
	return generate_password.slice(0, length)
									.split('').sort(() => Math.random() - 0.5)
									.join('');
}

function disableOnlyCheckbox(){
	let totalChecked = [uppercase_element, lowercase_element, number_element, symbol_element].filter(el => el.checked)
...
[uppercase_element, lowercase_element, number_element, symbol_element].forEach(el => {
